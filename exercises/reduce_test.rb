...
class ReduceTest < Minitest::Test
  ...
  def test_multiply_list_of_numbers
    numbers = [2, 3, 5, 7]
    result = numbers.inject(1){|product, number| product * number}
  ...
  def test_divide_560_by_a_bunch_of_numbers
    numbers = [2, 2, 2, 5, 7]
    result = numbers.inject(560){|accumulator, number| accumulator / number}
    assert_equal 2, result
  end

  def test_subtract_smallest_values_from_100
    elements = [[8, 5, 3], [1, 9, 11], [4, 7, 2], [19, 34, 6]]
    result = elements.reduce(100){|accumulator, element| accumulator - element.min}
    assert_equal 88, result
  end

  def test_add_all_the_second_values_together
    elements = [["a", 1], ["b", 9], ["c", 21]]
    result = elements.inject(0){|accumulator, element| accumulator + element.last}
