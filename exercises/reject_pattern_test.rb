...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_vowels
    ...
    letters.each do |letter|
      remaining << letter unless %W[a e i o u].include?(letter)
    end
  ...
  def test_remove_words_ending_in_e
    ...
    selected = []
    words.each do |word|
      selected << word unless word.split('').last == 'e'
    end
  ...
  def test_remove_words_ending_in_ing
    ...
    selected = []
    words.each do |word|
      selected << word unless word.split('')[-3..-1].join('') == 'ing'
    end
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
    animals.each do |animal|
      remaining << animal unless %W[a e i o u].include?(animal.split('').first)
    end
